...
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    DataPoint cur;
    // Deals with different cases for the file
    while (stream >> cur) {
        // If file is small, simply return irself
        if (pq.size () < k) {
            pq.enqueue(cur);
        // Else check for every element and enqueue if priority is higher
    ...
    //Turn into a vector to be returned
    Vector<DataPoint> top_k_elements;
    while (!pq.isEmpty () ) {
        top_k_elements.insert(0, pq.dequeue());
    }
